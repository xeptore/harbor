...
class Program
{
    ...
    static int Main(string[] args)
    {
        ...
        configFileOption.Validators.Add(_ => OptionValidation.AcceptExistingOnly(configFileOption));
        ...
        importScriptArgument.Validators.Add(_ => ArgumentValidation.AcceptExistingOnly(importScriptArgument));
    ...
    internal static int Export(FileInfo configFile)
    {
        ...
        foreach (var (schema, tableNames) in config.Tables)
        {
            foreach (var tableName in tableNames)
            {
                ...
                if (table == null)
                {
                    Console.Error.WriteLine($"⚠️ Table '[{schema}].[{tableName}]' not found, skipping.");
                    continue;
                }

                Console.Error.WriteLine($"📝 Queued table [{schema}].[{tableName}]");
    ...
    internal static int Import(FileInfo configFile, FileInfo scriptFile)
    {
        ...
        Console.Error.WriteLine($"🔌 Connecting to server {config.Host}:{config.Port} as {config.User}...");
        ...
        using (var conn = new SqlConnection(masterConnBuilder.ConnectionString))
        {
            ...
            {
                Console.Error.WriteLine($"⚠️ Dropping existing database '{config.Database}'...");
                server.KillDatabase(config.Database);
            }

            Console.Error.WriteLine($"🧱 Creating new database '{config.Database}'...");
