...
class Program
{
    ...
    static int Main(string[] args)
    {
        ...
        Argument<string> importScriptArgument = new("script")
        {
            Description = "Path to the SQL schema script file to import, or - to read from stdin",
            Arity = ArgumentArity.ExactlyOne,
        };
        importScriptArgument.Validators.Add(res =>
        {
            var filePath = res.Tokens.Single().Value;
            if (filePath == "-")
            {
                return;
            }
            ...
            {
                res.AddError($"File does not exist: '{filePath}'.");
                return;
            }
        });
    ...
    {
        Console.WriteLine("📤 Exporting schema to schema.sql...");
        ...
        {
            Console.WriteLine("❌ No tables specified in configuration.");
            return 1;
        }

        Console.WriteLine($"🔌 Connecting to {config.Host}:{config.Port} as {config.User}...");
        ...
        foreach (var (schema, tableNames) in config.Tables)
        {
            foreach (var tableName in tableNames)
            {
                ...
                if (table == null)
                {
                    Console.WriteLine($"⚠️ Table '[{schema}].[{tableName}]' not found, skipping.");
                    continue;
                }

                Console.WriteLine($"📝 Queued table [{schema}].[{tableName}]");
        ...
        {
            Console.WriteLine("❌ No valid tables found to script.");
            return 1;
        }
    ...
    internal static int Import(FileInfo configFile, string scriptPath)
    {
        if (scriptPath == "-")
        {
            Console.WriteLine("📥 Importing schema from standard input...");
        }
        else
        {
            Console.WriteLine($"📥 Importing schema from {scriptPath}...");
        }
        ...
        Console.WriteLine($"🔌 Connecting to server {config.Host}:{config.Port} as {config.User}...");
        ...
        using (var conn = new SqlConnection(masterConnBuilder.ConnectionString))
        {
            ...
            {
                Console.WriteLine($"⚠️ Dropping existing database '{config.Database}'...");
                server.KillDatabase(config.Database);
            }

            Console.WriteLine($"🧱 Creating new database '{config.Database}'...");
